...
date: '2018-11-14'
params:
  birthday: 你的出生西元年月日，用YYYY-DD-MM格式
  id: 你學號
  name: 你的名字
---
# 電子書
...
```{r ans14}
hwGrades[,-c(1)] # hwGrades[, c(2:4)]

is.na(hwGrades$hw1) -> aa
!is.na(hwGrades$hw1) -> bb

hwGrades$交作業次數 <- as.numeric(!is.na(hwGrades$hw1))+
  as.numeric(!is.na(hwGrades$hw2))+
  as.numeric(!is.na(hwGrades$Exercise3))
```
...
rowSums(hwGrades2) -> hwGrades2$交作業次數
```
...
```{r ans15}
rowSums(hwGrades[,c(2:4)],na.rm=T)/3 -> hwGrades$平均成績
#cbind(hwGrades[,c(2:4)],rowSums(hwGrades[,c(2:4)],na.rm=T))
```
...
```{r ans16}
(hwGrades$學號 %in% classSurvey$學號)-> aaa

hwGrades[aaa,]-> hwGradeSubsample
```
...
#hwGradeSubsample$學號 %in% classSurvey$女性學號
